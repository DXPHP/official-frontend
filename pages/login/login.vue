<template>
	<view class="template-login2">
      ...
      <view class="login__bg login__bg--top">
        <image class="rocket rocket-sussuspension" src="https://resource.tuniaokj.com/images/login/1/login_top3.png" mode="widthFix"></image>
			...
			<view class="login__wrapper">
				...
				<view class="login__info tn-flex tn-flex-direction-column tn-flex-col-center tn-flex-row-center">
						...
						<view
							...
							<view class="login__info__item__input__content">
								<input :password="!showPassword" placeholder-class="input-placeholder"
									v-model="signIn.password" placeholder="请输入登录密码" />
							</view>
					...
					<block v-if="currentModeIndex === 1">
						...
						<view
							...
							<view class="login__info__item__input__content">
								<input :password="!showPassword" placeholder-class="input-placeholder"
									v-model="signUp.password" placeholder="请输入登录密码" />
							</view>
							<view class="login__info__item__input__right-icon" @click="showPassword = !showPassword">
								<view :class="[showPassword ? 'tn-icon-eye' : 'tn-icon-eye-hide']"></view>
							</view>
					...
					<view class="login__info__item__button tn-cool-bg-color-7--reverse" hover-class="tn-hover"
						@click="accountLogin" :hover-stay-time="150">{{ currentModeIndex === 0 ? '登录' : '注册'}}</view>
				...
				<view class="login__way tn-flex tn-flex-col-center tn-flex-row-center">
					<view class="tn-padding-sm tn-margin-xs" @click="wexinLogin">
						<view
		...
		<!-- 验证码倒计时 -->
		<tn-verification-code ref="code" uniqueKey="login-demo-2" :seconds="60" @change="codeChange">
	...
	export default {
		...
		methods: {
			...
			wexinLogin(e) {
				...
				// if (!this.checked) return uni.$u.toast('请勾选已阅读并同意隐私政策')
				uni.login({
					success: (loginRes) => {
						that.$tn.message.loading('登录中')
						// 获取用户信息
						uni.getUserInfo({
							provider: 'weixin',
							success: async (infoRes) => {
								...
								setStorage(storage_userInfo,
									JSON.stringify(
										res_user
										.data))
								that.$tn.message.toast(res_token.data.msg)
								setTimeout(function() {
									uni.reLaunch({
										url: '/pages/index'
									})
